import { listResolutions, countResolutions, findResolutionById, createResolution, updateResolution, deleteResolution, slugExistsInResolutions, } from '../models/resolution.model.js';
...
export async function listResolutionsController(req, res, next) {
    try {
...
export async function getResolutionController(req, res, next) {
    try {
        const id = parseUuidParam(req.params.id);
        const item = await findResolutionById(id);
        if (!item)
            throw new AppError(404, 'Resolución no encontrada.');
...
export async function createResolutionController(req, res, next) {
    try {
...
export async function updateResolutionController(req, res, next) {
    try {
        const actor = req.session.user;
        const id = parseUuidParam(req.params.id);
...
export async function deleteResolutionController(req, res, next) {
    try {
        const actor = req.session.user;
        const id = parseUuidParam(req.params.id);
        const existing = await findResolutionById(id);
