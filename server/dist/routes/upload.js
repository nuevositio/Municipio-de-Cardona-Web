...
import { uploadImage as uploadImageToStorage, uploadDocument, } from '../services/storage.service.js';
...
uploadRouter.post('/image', requireAuth, (req, res, next) => {
    ...
    try {
        const result = await uploadImageToStorage(req.file.buffer, req.file.originalname, req.file.mimetype, req.file.size);
...
uploadRouter.post('/pdf', requireAuth, (req, res, next) => {
    ...
    try {
        const result = await uploadDocument(req.file.buffer, req.file.originalname, req.file.mimetype, req.file.size);
        res.status(201).json({ url: result.url, path: result.path });
    }
    catch (err) {
