...
export async function listNewsController(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function getNewsController(
  ...
  try {
    ...
    if (!item) throw new AppError(404, 'Noticia no encontrada.')
...
export async function updateNewsController(
  ...
  try {
    ...
    if (!existing) throw new AppError(404, 'Noticia no encontrada.')
    ...
    const updateInput: Parameters<typeof updateNews>[1] = {
      ...restData,
      updatedBy: actor.id,
    }
    ...
    if (!updated) throw new AppError(404, 'Noticia no encontrada.')
...
export async function deleteNewsController(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
