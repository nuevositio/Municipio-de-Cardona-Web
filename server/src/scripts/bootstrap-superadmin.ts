...
const envSchema = z.object({
  SUPERADMIN_USERNAME: z
    ...
    .regex(
      /^[a-zA-Z0-9_.-]+$/,
      'El nombre de usuario solo puede contener letras, números, guiones bajos, puntos y guiones',
    ),
  SUPERADMIN_EMAIL: z
    .string()
...
async function bootstrap(): Promise<void> {
  ...
  if (existingByUsername) {
    if (existingByUsername.role === 'superadmin') {
      console.log(
        `[bootstrap] ✔  El superadmin "${SUPERADMIN_USERNAME}" ya existe. No se realizaron cambios.\n`,
      )
    } else {
      console.warn(
        `[bootstrap] ⚠  El username "${SUPERADMIN_USERNAME}" ya está registrado con role "${existingByUsername.role}". No se modificó.\n`,
      )
    }
    process.exit(0)
  }
  ...
  if (existingByEmail) {
    console.warn(
      `[bootstrap] ⚠  El email "${SUPERADMIN_EMAIL}" ya está registrado para el usuario "${existingByEmail.username}". No se realizaron cambios.\n`,
    )
    process.exit(0)
  }
  ...
  if (existingSuperadmin) {
    console.warn(
      `[bootstrap] ⚠  Ya existe un superadmin en el sistema: "${existingSuperadmin.username}" (id=${existingSuperadmin.id}).\n` +
