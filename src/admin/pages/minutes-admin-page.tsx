...
import { Eye, EyeOff, FileText, Loader2, Paperclip, Pencil, Plus, Trash2, X } from 'lucide-react'
...
import { apiGetMinutes, apiCreateMinute, apiUpdateMinute, apiDeleteMinute } from '../api/minutes.js'
import { apiUploadPdf } from '../api/upload.js'
import { minuteFormSchema, type MinuteFormValues } from '../schemas/content.js'
...
export function MinutesAdminPage() {
  ...
  async function load() {
    ...
    setLoadError(null)
    try {
      const data = await apiGetMinutes(50, 0)
      setItems(data.items)
      setTotal(data.total)
    } catch (err) {
      setLoadError(err instanceof Error ? err.message : 'Error al cargar actas.')
    } finally {
      setLoading(false)
    }
  }
  ...
  function openEdit(item: MinuteItem) { setSelected(item); setSheetMode('edit') }
  ...
  async function handleDeleteConfirm() {
    ...
    setDeleteError(null)
    try {
      await apiDeleteMinute(deletingId)
      setDeletingId(null)
      void load()
    } catch (err) {
  ...
  async function handleTogglePublish(item: MinuteItem) {
    const newPublishedAt = item.publishedAt ? null : new Date().toISOString().slice(0, 10)
    try {
      await apiUpdateMinute(item.id, { publishedAt: newPublishedAt })
      void load()
    } catch {
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          <h1 className="font-heading text-2xl font-bold text-[--ink-900]">Actas de Concejo</h1>
          <p className="mt-0.5 text-sm text-[--ink-600]">{total} acta{total !== 1 ? 's' : ''}</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {!loading && !loadError && items.length === 0 && (
        <EmptyState
          title="Sin actas"
          description="Publica la primera acta del Concejo Deliberante."
          icon={FileText}
          action={
      ...
      <AdminSheet
        open={sheetMode !== null}
        onClose={closeSheet}
        title={sheetMode === 'create' ? 'Nueva acta' : 'Editar acta'}
      >
        <MinuteForm
          key={selected?.id ?? 'new'}
          selected={selected}
...
// ─────────────────────────────────────────────────────────────────────────────
// Formulario de acta (crear / editar)
// ─────────────────────────────────────────────────────────────────────────────

function MinuteForm({
  selected, mode, onSaved, onClose,
}: { selected: MinuteItem | null; mode: SheetMode; onSaved: () => void; onClose: () => void }) {
  const { register, handleSubmit, setValue, formState: { errors, isSubmitting } } = useForm<MinuteFormValues>({
  ...
  async function onSubmit(values: MinuteFormValues) {
    setSubmitError(null)
    try {
      let fileUrl = values.fileUrl

      // Si hay un PDF nuevo, subirlo primero
      if (pdfFile) {
      ...
      if (mode === 'create') {
        await apiCreateMinute(payload)
      } else if (selected) {
        await apiUpdateMinute(selected.id, payload)
      }
      onSaved()
    } catch (err) {
  ...
  return (
    <form onSubmit={(e) => void handleSubmit(onSubmit)(e)} className="space-y-4">
      <FormField label="Título" required error={errors.title?.message}>
        <input type="text" placeholder="Ej. Acta N° 12 – Sesión ordinaria" {...register('title')} className={inputCls(!!errors.title)} />
      </FormField>
      ...
      <FormField label="Resumen (opcional)" error={errors.summary?.message}>
        <textarea rows={3} placeholder="Breve descripción del acta…" {...register('summary')} className={inputCls(!!errors.summary)} />
      </FormField>
